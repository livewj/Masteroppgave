function [b0, dt, w, w_wlls] = dti_fit(dwi, grad, mask, constraints, outliers, maxbval)
    % Diffusion Tensor Imaging tensor estimation using 
    ...
    % Usage:
    % ------
    % [b0, dt] = dti_fit(dwi, grad [, mask [, constraints]])
    %  
    %
    % b0 = signal at b=0
    % dt = Diffusion tensor
    ...
    %       prior to tensor fitting. Thes steps are not included in this
    ...
    %       default: [1 0 0]
    %
    %     5. maxbval (scalar; default = 3.0ms/um^2), puts an upper bound on the b-values being
    ...
    %% limit DTI fit to b=3000
    bval = grad(:, 4); %b verdiene
    order = floor(log(abs(max(bval)+1))./log(10)); %sjekker max b sin str orden
    if order >= 2
        grad(:, 4) = grad(:, 4)/1000; %deler b på 1000
        bval = grad(:, 4);
    end
    
    if ~exist('maxbval','var') || isempty(maxbval)
        maxbval = 3; %Setter max b lik 3000
    end
    list = bval<=maxbval; 
    dwi = dwi(:,:,:,list);
    grad = grad(list, :);
    ...
    %b = [bS, -bval(:, ones(1, 6)).*bD, (bval(:, ones(1, 15)).^2/6).*bW]; %D and W tensor
    b = [bS, -bval(:, ones(1, 6)).*bD]; %D tensor


    % Unconstrained LLS fit
    dt = b\log(dwi); % = pinv(b)*log(dwi)  Moore-Penrose pseudoinverse
    w = exp(b*dt); % signal

    nvoxels = size(dwi,2);

    % Constrained WLLS fit initialized with LLS
    if any(constraints) 
        ...
        ndir = size(dir, 1);
        C = [];
        if constraints(1)>0 %TRUE
            %C = [C; [zeros(ndir, 1), D_cnt(ones(ndir, 1), :).*dir(:,D_ind(:, 1)).*dir(:,D_ind(:, 2)), zeros(ndir, 15)]];
            C = [C; [zeros(ndir, 1), D_cnt(ones(ndir, 1), :).*dir(:,D_ind(:, 1)).*dir(:,D_ind(:, 2))]];
        end
        if constraints(2)>0 
        ...
        d = zeros([1, size(C, 1)]);
        options = optimset('Display', 'off', 'Algorithm', 'interior-point', 'MaxIter', 5000, 'TolCon', 1e-10, 'TolFun', 1e-10, 'TolX', 1e-10, 'MaxFunEvals', 5000);
    ...
    dt_wlls = [dt_0; dt];
    w_wlls = exp(b*dt_wlls); % Signal estimated from WLLS
